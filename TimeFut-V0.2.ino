...
// Versão: 0.2 - 25/03/2015 - Tempo de jogo configurável     //
// Versão: 0.1 - 07/04/2014 - Firmware OK                    //
//                                                           //
//************** Funcionalidades ****************************//
// - Faz a contagem do tempo de uma partida                  //
// - O tempo de jogo pode ser configurado ligando o sistema  //
//   com o botão PLAY pressionado, depois mudando o tempo    //
//   com os botões PLAY e PAUSE e salvando com o RESET       //
...
#include <EEPROM.h>                                          // Biblioteca para usar a EEPROM interna do arduino
...
#define  SEM_ALERTA     0x00                                 // Possíveis estados do alerta (statusAlerta)
...
#define  TEMPO_JOGO_DEFAULT    600                           // 10 minutos = 10 x 60 segundos = 600
#define  TEMPO_MAX     TEMPO_MINUTO * 45                     // Tempo máximo da partida (45 minutos)
#define  TEMPO_MIN     TEMPO_MINUTO * 1                      // Tempo mínimo da partida (1 minutos)
#define  BASE10        10                                    // Base para calcular o numero a ser mostrado no display

#define EEPROM_ADDRESS_HIGH     0x01                        // Endereços da EEPROM onde é armazenado o tempo de jogo
#define EEPROM_ADDRESS_LOW      0x02

                                                             // Valores de 0 a 9 para o display de 7 segmentos (o = Aceso / 1 = Apagado)
byte  decTO7seg[10] = {B00000011,                            // ZERO_7SEG
                       B10011110,                            // UM_7SEG
                       B00100101,                            // DOIS_7SEG
                       B00001100,                            // TRES_7SEG
                       B10011001,                            // QUATRO_7SEG
...
unsigned int latchPin = 8;                                   // Latch, conectado ao pino 12 do 74HC595
unsigned int dataPin = 11;                                   // Data, conectado ao pino 14 do 74HC595
unsigned int clockPin = 12;                                  // Clock, conectado ao pino 11 do 74HC595
unsigned int alertaPin = 13;                                 // Pino que controla a sirene
unsigned int botao_PAUSE = 5;                                // Botão play do controle
unsigned int botao_RESET = 6;                                // Botão pause do controle
unsigned int botao_PLAY = 7;                                 // Botão reset do controle
unsigned int ledPin = 9;                                     // Led que acende a cada segundo
unsigned int disp1Pin = 1;                                   // Pino que controle o display 1
unsigned int disp2Pin = 2;                                   // Pino que controle o display 2
unsigned int disp3Pin = 3;                                   // Pino que controle o display 3
unsigned int disp4Pin = 4;                                   // Pino que controle o display 4
...
unsigned int contTime;                                       // Variável para contar o tempo de jogo
unsigned int contSeg;                                        // Variável para contar 1 segundo
unsigned int minTemp, segTemp;                               // Variáveis para armazenar o valor dos segundos e minutos na forma decimal
...
//************************** Setup **************************//
void setup() {                                               // Loop de configuração                                    
  ...
  contTime = word(EEPROM.read(EEPROM_ADDRESS_HIGH), EEPROM.read(EEPROM_ADDRESS_LOW));   // Le o tempo de jogo salvo na EEPROM
  if(contTime == 0)  contTime = TEMPO_JOGO_DEFAULT;          // Se tempo de jogo na EEPROM estiver zerado, carrega tempo default
...
void loop() {                                                // Loop principal  
  if(digitalRead(botao_PLAY) == HIGH) ajusta_tempo_jogo();   // Se botão PLAY pressionado, entra na função de configuração do tempo da partida
...
//******************* ajusta_tempo_jogo *********************//
void ajusta_tempo_jogo(void){                                // Função para ajustar tempo da partiga
  contTime = TEMPO_JOGO_DEFAULT;                             // Inicializa com tempo default
  atualiza_contador();                                       // Atualiza os digitos do display
  while(digitalRead(botao_PLAY) == HIGH);                    // Aguarda botão PLAY ser solto
  delay(100);                                                // Debounce para o botão
  
  while(digitalRead(botao_RESET) == LOW){                    // Enquanto o botão RESET não for pressionado, fica monitorando os botões
    if(digitalRead(botao_PLAY) == HIGH){                     // Se botão PLAY for pressionado...
      if(contTime < TEMPO_MAX){                              // Se tempo de jogo menor que tempo máximo...
        contTime += TEMPO_MINUTO;                            // Acrescenta 1 minuto no tempo da partida
        atualiza_contador();                                 // Atualiza os digitos do display
        while(digitalRead(botao_PLAY) == HIGH);              // Aguarda botão PLAY ser solto
        delay(100);                                          // Debounce para o botão
      }      
    }
    if(digitalRead(botao_PAUSE) == HIGH){                    // Se botão PAUSE for pressionado...
      if(contTime > TEMPO_MIN){                              // Se tempo de jogo maior que tempo mínimo...
        contTime -= TEMPO_MINUTO;                            // Decrementa 1 minuto no tempo da partida
        atualiza_contador();                                 // Atualiza os digitos do display
        while(digitalRead(botao_PAUSE) == HIGH);             // Aguarda botão PAUSE ser solto
        delay(100);                                          // Debounce para o botão
      }      
    }    
  } 
 EEPROM.write(EEPROM_ADDRESS_HIGH, highByte(contTime));      // Escreve na EEPROM o byte alto do tempo da partida
 EEPROM.write(EEPROM_ADDRESS_LOW, lowByte(contTime));        // Escreve na EEPROM o byte baixo do tempo da partida
 
 while(digitalRead(botao_RESET) == LOW);                      // Aguarda botão RESET ser solto
  delay(100);                                                 // Debounce para o botão
}
